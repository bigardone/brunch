...
const concat = (files, path, definition, autoRequire, config) => {
  ...
  const hasRequire = file => {
    // 'require' detection sometimes does not work for some js files...
    return true;
  };
  const moduleFiles = nonNpmFiles.filter(hasRequire);
  const nonModuleFiles = nonNpmFiles.filter(f => moduleFiles.indexOf(f) === -1);
  const isHmr = isJs && (moduleFiles.length > 0 || npmFiles.length > 0);
...
const generate = (path, targets, config, optimizers) => {
  ...
  return optimize(code, map, path, foptim, targets)
    ...
    .then(data => {
      return common.writeFile(path, data.code).then(() => data);
    })
    .then(data => {
      if (withMaps) {
        return common.writeFile(mapPath, data.map.toString()).then(() => data);
      }
      return data;
    });
};
